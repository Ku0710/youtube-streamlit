...
def get_df_ec():
    ...
    item_list = soup.find('ul', {'id': 'item_list'})
    items = item_list.find_all('li')

    data_ec = []
    for item in items:
        datum_ec = {}
        datum_ec['title'] = item.find('p', {'class': 'items_grid_itemTitleText_b5866da'}).text
        price = item.find('p', {'class': 'items_grid_price_b5866da'}).text
        datum_ec['price'] = int(price.replace(('¥ ', '')).replace(',', ''))
        datum_ec['link'] = item.find('a')['href']
        is_stock = item.find('p', {'class': 'items-grid-soldOut_b58666da'}) == None
